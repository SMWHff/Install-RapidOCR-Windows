...
$('run').onclick=async()=>{
  ...
  try{
    ...
    if(d.error){$('out').innerHTML='<div class="empty">'+esc(d.error)+'</div>';$('text').value='';draw([]);return;}
    ...
    $('text').value=groupText(d.result);
  }catch(e){$('out').innerHTML='<div class="empty">&#x8bf7;&#x6c42;&#x5931;&#x8d25;: '+esc(e.message)+'</div>';$('text').value='';}
